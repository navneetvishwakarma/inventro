...
- `docs/00-working-spec.md` — pre-existing consolidated spec, predates the doc
  set below. Source of truth for the prediction algorithm (§5), domain model
  (§3), LLM extraction contract (§7), and validation harness (§12) — read
  before touching any of those. Where it and the PRD drift, `REQ-xx` ids win
  for scope/priority; this doc wins for algorithm/schema detail.
