...
export async function wipeDemoHouseholdData(): Promise<WipeResult> {
  ...
  // id, so there is no code path by which the real household could be
  ...
  if (DEMO_HOUSEHOLD_ID === getDefaultHouseholdId()) {
    throw new Error('DEMO_HOUSEHOLD_ID must not equal the real household id -- refusing to wipe');
  }
