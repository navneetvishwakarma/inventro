-- S-13: commit is one atomic function so a mid-loop failure never leaves a
-- receipt half-applied to the ledger (no partial stock_movements/
-- catalog_items for a receipt that ends up not 'committed'). Re-validates
-- the same guards S-12's UI already enforces (purchase date confirmed, no
-- needs_review lines left) independently, server-side.
--
-- matched lines trust matched_item_id/qty_base exactly as S-09/S-10 (or a
-- S-12 "Save & match" edit) already left them -- this function never calls
-- the matching ladder itself. That's deliberate: re-matching inside this
-- loop would let a same-receipt new_item line's freshly-inserted alias
-- (inserted by an EARLIER iteration of this same loop) trigram-match a
-- LATER, genuinely different line -- caught in this epic's advisor review
-- before this was written.
--
-- new_item qty_base is computed in TypeScript (lib/receipts/canonicalize.ts's
-- normalizeUnitToBase, already exercised by S-10) and passed in via
-- p_new_item_qty_base rather than reimplemented in SQL, so the one piece of
-- non-trivial unit-parsing logic has a single source of truth.
