-- S-31 fix (caught during this story's own live-Supabase verification,
-- before commit): Postgres substring(text from pattern) returns the match
-- of the FIRST PARENTHESIZED SUBEXPRESSION when the pattern contains one,
-- not the whole match. The prior migration's pattern
-- '^[0-9]+(\.[0-9]+)?' has its only group wrapping the OPTIONAL decimal
-- part -- for a plain integer qty_display like '2' (no decimal), that
-- group doesn't participate in the match, so substring returned NULL
-- instead of '2', silently disabling the secondary fallback for the exact
-- common case (whole-number quantities) it exists to cover. Fixed by
-- wrapping the ENTIRE leading-numeric match in the outer (first) group:
-- '^([0-9]+(\.[0-9]+)?)' -- substring now returns group 1, the whole
-- leading number, with the inner group only used for the optional decimal
-- syntax, never returned itself. Verified live: qty_display='2' now
-- resolves to 2, not NULL.
