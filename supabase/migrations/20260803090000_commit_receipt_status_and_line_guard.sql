-- S-67/E-22: commit_receipt() silently marked a receipt 'committed' with
-- zero movements written when it had zero matched/new_item lines (needs_review
-- was already guarded against, but a receipt with only excluded lines, or no
-- lines at all, sailed through). It also never checked receipts.status --
-- a 'pending' (still-processing or failed-extraction) receipt could be
-- committed if a client ever called this RPC directly, bypassing the UI's
-- own gating entirely. Defense in depth per this story's own invariant: the
-- UI disabling the Commit button is not enough on its own.
...
begin
  ...
  if v_receipt.status <> 'parsed' then
    raise exception 'receipt % is not ready to commit (status=%)', p_receipt_id, v_receipt.status;
  end if;
  ...
  if not exists (select 1 from receipt_lines where receipt_id = p_receipt_id and review_state in ('matched', 'new_item')) then
    raise exception 'receipt % has no matched or new-item lines to commit', p_receipt_id;
  end if;
